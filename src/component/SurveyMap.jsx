/**
 * SurveyMap.jsx — SurveyMap Pro v5.1.1
 * ─────────────────────────────────────────────────────────────────────────────
 * FIXES IN THIS VERSION:
 *
 * [v5.1.1] CONTOUR GREY GRID FIX ← THIS VERSION
 *   Problem: Grey broken-image grid squares appear on Contour (and any layer
 *   with an overlayUrl) because the overlay TileLayer was requesting tiles at
 *   zoom levels the server doesn't have.
 *
 *   Root cause: The old code used || for the maxNativeZoom fallback chain:
 *     maxNativeZoom={cfg.overlayMaxNativeZoom || cfg.maxNativeZoom || 19}
 *   The Contour overlay has overlayMaxNativeZoom=15 (correct, truthy), BUT
 *   the offline branch was incorrectly using cfg.maxNativeZoom (base layer's
 *   value = 17) instead of cfg.overlayMaxNativeZoom for the overlay.
 *   So offline Contour always requested tiles up to z17 from a z15 server.
 *
 *   Fix: Pre-compute zoom caps ONCE above the JSX using ?? (nullish coalescing):
 *     const baseMaxNative    = cfg.maxNativeZoom    ?? 19
 *     const overlayMaxNative = cfg.overlayMaxNativeZoom ?? cfg.maxNativeZoom ?? 17
 *   Both online and offline overlay layers now use overlayMaxNative.
 *   Final fallback is 17 (not 19) because overlay servers (contour, hillshade,
 *   topo) almost always cap at z17 or lower — 19 was too aggressive.
 *
 * [v5.1.0] BLUR FIX (carried forward)
 *   - setMeasureUnit + onFinish now passed to MeasureTool
 *   - backdropFilter removed from sidebar (was creating stacking context)
 *   - overflow:hidden removed from root div
 */
...
export default function SurveyMap() {
  ...
  const { cursorElevation, getCursorElevation, getElevationProfile } = useElevation({ isOnline });
  ...
  const handleKMLUpload   = (e) => { const f=e.target.files[0]; if(!f) return; setKmlLoading(true); setKmlName(f.name); setKmlFile(f); e.target.value=""; };
  const handleExtraUpload = (e) => { const f=e.target.files[0]; if(!f) return; const ext=f.name.split(".").pop().toLowerCase(); if(ext!=="kmz"&&ext!=="csv"){alert("Please upload a KMZ or CSV file.");e.target.value="";return;} setExtraFile(f); setExtraFileType(ext); e.target.value=""; };
  ...
  const finishDrawing  = () => { if(!drawPoints.length) return; setPendingPoints(drawPoints); setPendingType(drawType); setPendingName(""); setShowNameModal(true); };
  ...
  /* ════════════════════════════════════════════════════════════════════════════
     ZOOM CAP COMPUTATION
     ✅ FIX: computed once here, used in BOTH online and offline tile branches.
     ?? only falls through on null/undefined (not on 0 or false).
     Overlay final fallback = 17 (not 19) — overlay servers cap low.
  ════════════════════════════════════════════════════════════════════════════ */
  const baseMaxNative    = cfg.maxNativeZoom    ?? 19;
  const overlayMaxNative = cfg.overlayMaxNativeZoom ?? cfg.maxNativeZoom ?? 17;
      ...
      <div style={{ position:"fixed", inset:0, background:"#060e1a", fontFamily:"'DM Sans',sans-serif" }}>
        ...
        {!isMobile && (
          <div className="sm-sidebar" style={{ position:"absolute", top:TOP_H, left:0, width:SB_W, bottom:STAT_H, zIndex:1100,
            background:"rgba(4,10,22,0.99)", /* ✅ NO backdropFilter */ borderRight:"1px solid rgba(255,255,255,0.07)",
                ...
                <div style={{padding:"8px 12px 6px"}}>
                  <div style={{display:"flex",gap:5,marginBottom:8}}>
                    <label style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"7px 10px",borderRadius:8,cursor:"pointer",background:"rgba(20,184,166,0.12)",border:"1px solid rgba(20,184,166,0.3)",color:"#2dd4bf",fontSize:11.5,fontWeight:600}}>
                      <Ico name="Upload" size={12}/>Import<input type="file" accept=".geojson,.json" onChange={geoJSON.handleGeoJSONUpload} style={{display:"none"}}/>
                    </label>
        ...
        {!isMobile && (
          <div style={{position:"absolute",top:0,left:0,right:0,height:MENU_H,zIndex:1200,background:"rgba(5,12,24,0.97)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.055)",display:"flex",alignItems:"center",paddingLeft:12,gap:0}}>
            ...
            <button style={{display:"flex",alignItems:"center",gap:6,padding:"4px 12px",borderRadius:6,border:"1px solid rgba(74,158,255,0.3)",background:"rgba(74,158,255,0.12)",color:"#80c4ff",cursor:"pointer",fontSize:11,fontWeight:600,marginRight:10}}><Ico name="User" size={12}/> Sign In</button>
          </div>
        )}
        ...
        {!isMobile && (
          <div style={{position:"absolute",top:MENU_H,left:0,right:0,height:TB_H,zIndex:1150,background:"rgba(5,12,24,0.90)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.09)",display:"flex",alignItems:"center",padding:"0 10px",gap:5,overflowX:"auto"}}>
            {[{key:"Satellite",icon:"Satellite",short:"Satellite"},{key:"Street",icon:"Street",short:"Street"},{key:"Terrain",icon:"Terrain",short:"Terrain"},{key:"Satellite + Labels",icon:"SatLabels",short:"+Labels"},{key:"Dark",icon:"Dark",short:"Dark"},{key:"Light",icon:"Light",short:"Light"}]
              .map(({key,icon,short}) => <button key={key} className={`tb-btn ${activeLayer===key?"active":"inactive"}`} onClick={() => setActiveLayer(key)}><Ico name={icon} size={14}/><span>{short}</span></button>)}
            ...
            <div style={{width:1,height:22,background:"rgba(255,255,255,0.08)",margin:"0 3px",flexShrink:0}}/>
            <label className={`tb-btn ${geoJSON.importedGeoJSONLayers.length?"geojson-active":"inactive"}`} style={{cursor:"pointer"}} title="Import GeoJSON">
              <Ico name="GeoJSON" size={14}/><span>GeoJSON</span><input type="file" accept=".geojson,.json" onChange={geoJSON.handleGeoJSONUpload} style={{display:"none"}}/>
            </label>
        ...
        {isMobile&&<div style={{position:"absolute",bottom:70,right:14,zIndex:1200}}><MobileCompassWidget compassNavActive={compass.compassNavActive} compassHeading={compass.compassHeading} onCompassToggle={() => compass.compassNavActive?compass.stopCompassNav():compass.startCompassNav()} leafletMapRef={leafletMapRef}/></div>}

        {/* ═══════════════════════════════════════════════════════════════════
            MAP  —  NO overflow:hidden (clips MeasureTool panel)
        ═══════════════════════════════════════════════════════════════════ */}
        <div style={{position:"absolute",top:mapTop,left:mapLeft,right:0,bottom:mapBottom,zIndex:1}}>
          <MapContainer center={[20.29,85.82]} zoom={13} maxZoom={22} zoomControl={false} style={{width:"100%",height:"100%"}}
            ...
            {/* ── TILE LAYERS ────────────────────────────────────────────────
                ✅ GREY GRID FIX: both branches now use:
                  base    → baseMaxNative    (cfg.maxNativeZoom ?? 19)
                  overlay → overlayMaxNative (cfg.overlayMaxNativeZoom ?? cfg.maxNativeZoom ?? 17)
                These are computed above using ?? so no value is accidentally
                skipped. overlayMaxNative for Contour resolves to 15 (correct).
            ──────────────────────────────────────────────────────────────── */}
            {cfg.type==="wms"
              ...
              : offlineMode
                ? <>
                    <OfflineTileLayer key={activeLayer} layerKey={activeLayer} url={cfg.url} attribution={cfg.attribution}
                      offlineOnly={offlineMode} maxZoom={22} maxNativeZoom={baseMaxNative}/>
                    {cfg.overlayUrl&&(
                      <OfflineTileLayer key={activeLayer+"_overlay"} layerKey={activeLayer+"_overlay"} url={cfg.overlayUrl}
                        offlineOnly={offlineMode} maxZoom={22}
                        maxNativeZoom={overlayMaxNative}
                        opacity={cfg.overlayOpacity??(activeLayer==="Contour"?0.75:0.85)}/>
                    )}
                  </>
                : <>
                    <TileLayer key={activeLayer} url={cfg.url||"https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"}
                      attribution={cfg.attribution||"© OpenStreetMap contributors"}
                      maxZoom={22} maxNativeZoom={baseMaxNative} crossOrigin="anonymous"/>
                    {cfg.overlayUrl&&(
                      <TileLayer key={activeLayer+"_overlay"} url={cfg.overlayUrl}
                        maxZoom={22}
                        maxNativeZoom={overlayMaxNative}
                        opacity={cfg.overlayOpacity??(activeLayer==="Contour"?0.75:0.85)}
        ...
        {/* Mobile bottom nav */}
        {isMobile&&<div style={{position:"absolute",bottom:0,left:0,right:0,zIndex:1200}}><MobileBottomNav onOpen={setActiveSheet} onCompassToggle={() => compass.compassNavActive?compass.stopCompassNav():compass.startCompassNav()} compassNavActive={compass.compassNavActive} drawMode={drawMode} measureMode={measureMode} surveyMode={surveyMode} isTracking={isTracking}/></div>}
            ...
            <div style={{paddingBottom:28}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,padding:"4px 16px"}}>
                {Object.entries(MAP_LAYERS).map(([name,layer]) => {
                  ...
                  return <button key={name} onClick={() => {setActiveLayer(name);setActiveSheet(null);}} style={{padding:"13px 12px",borderRadius:14,cursor:"pointer",background:on?"rgba(59,130,246,0.16)":"rgba(255,255,255,0.035)",border:`1.5px solid ${on?"rgba(59,130,246,0.5)":"rgba(255,255,255,0.07)"}`,display:"flex",flexDirection:"column",alignItems:"flex-start",gap:7,textAlign:"left"}}>
        ...
        {!isMobile&&(
          <div style={{position:"absolute",bottom:0,left:0,right:0,height:STAT_H,zIndex:1100,background:"rgba(4,10,20,0.94)",backdropFilter:"blur(12px)",borderTop:"1px solid rgba(255,255,255,0.055)",display:"flex",alignItems:"center",padding:"0 14px",gap:10,userSelect:"none"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4}}><span style={{animation:"blink 1.5s infinite",color:"#4a9eff",fontSize:8}}>●</span><span style={{color:"rgba(255,255,255,0.22)",fontSize:9.5}}>Live</span></div>
            </div>
          </div>
        )}
        ...
        {!isMobile&&<ElevationProfile visible={elevOpen} onClose={() => setElevOpen(false)} profileData={elevProfileData} loading={false} isOnline={isOnline} sourceLabel={elevSourceLabel} leafletMap={mapRefForTracker} activeMode={elevMode} onRequestPoints={handleElevModeRequest}/>}
        ...
        {locationInfo&&(
          <div style={{position:"absolute",top:mapTop+14,right:70,width:310,zIndex:1050,borderRadius:14,overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.7)",border:"1px solid rgba(255,255,255,0.09)",background:"rgba(5,12,24,0.97)",backdropFilter:"blur(24px)",fontFamily:"'DM Sans',sans-serif"}}>
            ...
            <div style={{padding:locationInfo.photo?"12px 16px 14px":"14px 16px"}}>
              ...
              {locationInfo.loading?<div style={{color:"rgba(255,255,255,0.28)",fontSize:11,fontStyle:"italic"}}>⏳ Fetching details…</div>
                :locationInfo.description?<div style={{color:"rgba(200,225,255,0.65)",fontSize:11.5,lineHeight:1.65,maxHeight:100,overflowY:"auto"}}>{locationInfo.description.slice(0,350)}{locationInfo.description.length>350?"…":""}</div>:null}
          ...
          <div style={{position:"fixed",inset:0,zIndex:3000,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",padding:"0 16px",backdropFilter:"blur(12px)"}}>
            <div style={{background:"rgba(5,12,24,0.98)",borderRadius:16,padding:28,width:"100%",maxWidth:360,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 24px 72px rgba(0,0,0,0.85)",border:"1px solid rgba(74,158,255,0.18)",fontFamily:"'DM Sans',sans-serif"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"center",marginBottom:14}}>
                <div style={{width:52,height:52,borderRadius:14,background:"linear-gradient(135deg,rgba(74,158,255,0.22),rgba(37,99,235,0.22))",border:"1px solid rgba(74,158,255,0.32)",display:"flex",alignItems:"center",justifyContent:"center"}}><Ico name="Compass" size={26} style={{color:"#4a9eff"}}/></div>
              </div>
              <div style={{color:"#c8e0f8",fontWeight:700,fontSize:20,textAlign:"center",marginBottom:5}}>SurveyMap Pro</div>
              <div style={{color:"rgba(255,255,255,0.32)",fontSize:12,textAlign:"center",marginBottom:18}}>Version 5.1.1 — Contour Grid Fix</div>
